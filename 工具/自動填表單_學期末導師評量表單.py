...
class auto_Table:
    ...
    def xpath(self, x, y):
        for i in range(x, y):
            try:
                driver.find_element_by_xpath('/html/body/form/table/tbody/tr[%d]/td[2]/input[2]' % (i)).click()
            except:
                continue
            return 0
...
while title == 'Campus Information Services':
    ...
    if title != 'Portal Index':
        ...
        print('帳號密碼或驗證碼輸入錯誤')
        driver.find_element_by_xpath('/html/body/div/div[1]/div/div[2]/div/div[3]/div[5]/div/div[2]/a').click()
        continue
...
while title == '教學評量系統':
    try:
        driver.find_element_by_xpath('/html/body/table/tbody/tr[%d]' % (table_count)).click()
        title = driver.title
        table_count += 1
    except:
        table_count += 1
    ...
    if title == '':
        driver.find_element_by_xpath('//*[@id="form1"]/div/table/tbody[2]/tr/td[4]/input[1]').click()
        title = driver.title
        if title == '105年第1學期教學成效問卷_學生':
            for i in range(1, 19):
                driver.find_element_by_xpath('//*[@id="questions"]/ol/li[%d]/ul/li[2]/input' % (i)).click()
